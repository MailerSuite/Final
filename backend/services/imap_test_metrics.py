"""
IMAP Test Service - Service for IMAP connection testing
...
class IMAPTestMetrics:
    def __init__(self):
        self.state = IMAPTestState.IDLE
        self.attempts = 0
        self.successes = 0
        self.failures = 0
...
class IMAPTestService:
    ...
    async def start(
        ...
        self.metrics = IMAPTestMetrics()
        self.metrics.state = IMAPTestState.RUNNING
    ...
    async def _runner(
        ...
        try:
            while not self._stop_event.is_set():
                ...
                log = IMAPLoginLog(datetime.utcnow(), success, response)
    ...
    async def _login_capability(
        ...
        try:
            ...
            return True, "* CAPABILITY IMAP4rev1"
    ...
    async def send_bulk(
        ...
        results = []
        for i in range(emails_per_item):
