...
const ProcessesPage: React.FC = () => {
  ...
  const fetchData = useCallback(async (searchQuery?: string, statusFilter?: string, offset = 0) => {
    if (!session?.id && !import.meta.env.DEV) return;
    ...
    try {
      ...
      const response = await axiosInstance.get<ProcessesResponse>(
        `/api/v1/processes?${params}`
      );
  ...
  // Load data on mount
  useEffect(() => {
    if (session?.id || import.meta.env.DEV) {
      fetchData();
    }
  ...
  if (!session?.id && !import.meta.env.DEV) {
    return (
      <PageWrapper title="Processes">
        <Alert>
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            Please log in to access Processes.
          </AlertDescription>
        </Alert>
      </PageWrapper>
    );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage and monitor processes operations
            </p>
          </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-muted-foreground">
                  Get started by creating your first processes item.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
