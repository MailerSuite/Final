...
const SecurityMonitoringPage: React.FC = () => {
  ...
  // State
  const [data, setData] = useState<SecurityMonitoringData[]>([]);
  ...
  const fetchData = useCallback(async (searchQuery?: string, statusFilter?: string, offset = 0) => {
    ...
    try {
      ...
      const response = await axiosInstance.get<SecurityMonitoringResponse>(
        `/api/v1/security-monitoring?${params}`
      );
      ...
      console.error('Error fetching security-monitoring data:', err);
      setError(err.response?.data?.message || 'Failed to fetch security-monitoring data');
      toast.error('Failed to fetch security-monitoring data');
    } finally {
      setLoading(false);
    }
  }, [session?.id]);
  ...
  if (!session?.id) {
    return (
      <PageWrapper title="Security Monitoring">
        <Alert>
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>
            Please log in to access Security Monitoring.
          </AlertDescription>
        </Alert>
      </PageWrapper>
    );
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">
              Manage and monitor security-monitoring operations
            </p>
          </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <div className="flex-1">
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Search security-monitoring..."
                    value={searchTerm}
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium text-muted-foreground mb-2">
                  No security-monitoring data available
                </h3>
                <p className="text-sm text-muted-foreground">
                  Get started by creating your first security-monitoring item.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
