...
export const ProductionReadinessDashboard: React.FC = () => {
  ...
  const getHealthBadge = (status: string) => {
    switch (status) {
      ...
      case 'online':
      case 'connected':
        return <Badge variant="default" aria-label="Status: Healthy">Healthy</Badge>;
      case 'warning':
      case 'degraded':
        return <Badge variant="secondary" aria-label="Status: Warning">Warning</Badge>;
      case 'critical':
      case 'offline':
      case 'disconnected':
        return <Badge variant="destructive" aria-label="Status: Critical">Critical</Badge>;
      default:
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="p-4 border rounded-lg">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant="default" aria-label="Services online count">
                    ...
                    <div className="space-y-2">
                      {Object.entries(systemHealth.services).map(([service, status]) => (
                        <div key={service} className="flex justify-between items-center">
                          <span className="capitalize">{service}:</span>
                          {status ? (
                            <Badge variant="default" aria-label={`${service} online`}>Online</Badge>
                          ) : (
                            <Badge variant="destructive" aria-label={`${service} offline`}>Offline</Badge>
                          )}
                        </div>
                      ))}
                    </div>
